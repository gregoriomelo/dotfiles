...
```bash
REF_DIR="$(find -L "$HOME/.agents/skills" "$HOME/.claude/skills" "$HOME/skills" -name "render_excalidraw.py" 2>/dev/null | head -n 1 | xargs dirname)"
uv run --directory "$REF_DIR" python "$REF_DIR/render_excalidraw.py" <path-to-file.excalidraw>
```
