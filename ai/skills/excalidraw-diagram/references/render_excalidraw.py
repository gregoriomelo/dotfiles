...
def render(
    ...
    try:
        ...
        print(f"Run: (cd '{ref_dir}' && uv sync && uv run playwright install chromium)", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print(f"Run: (cd '{ref_dir}' && uv run playwright install chromium)", file=sys.stderr)
                sys.exit(1)
            raise
        ...
        # Fulfill template under esm.sh origin so module and sub-resource imports resolve cleanly
        page.route(
            "https://esm.sh/__render.html",
        ...
        page.goto("https://esm.sh/__render.html")
...
def main() -> None:
    ...
    png_path = render(input_path, output_path, args.scale, args.width)
    print(str(png_path))
